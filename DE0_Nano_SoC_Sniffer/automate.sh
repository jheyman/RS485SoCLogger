...
PATH=$PATH:/usr/local/DS-5_v5.28.1/sw/gcc/bin
export PATH
echo $PATH
...
partition_sdcard() {
    ...
    # To handle the case of an SD card that was already created with these partitions,
    # un-mount the partitions (may have been auto-mounted, e.g. under Ubuntu) since
    # this can cause "device or resource busy" error in fdisk
    # This won't hurt if the partition did not exist / if partitions were not mounted.
    sudo umount "${sdcard_dev_fat32}"
    sudo umount "${sdcard_dev_ext3}"
