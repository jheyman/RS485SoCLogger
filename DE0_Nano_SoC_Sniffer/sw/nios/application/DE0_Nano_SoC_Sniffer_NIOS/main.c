...
{
	return IORD_FIFOED_AVALON_UART_RX_FIFO_USED(FIFOED_AVALON_UART_0_BASE);
}
...
void enable_led(bool onoff) {
	uint32_t led = IORD_ALTERA_AVALON_PIO_DATA(NIOS_ACTIVITY_LED_0_BASE);
	...
	IOWR_ALTERA_AVALON_PIO_DATA(NIOS_ACTIVITY_LED_0_BASE, led);
}
...
#define RAMDEST_UART_RXINFO_UART0     (void*)0x38000000
...
int main() {
	...
	int uart_fd;
	uart_fd = open(FIFOED_AVALON_UART_0_NAME, O_RDWR | O_NONBLOCK);
	...
	UART_RXinfo.RXBaseAddress = RAMDEST_UART0_RX_START;
	UART_RXinfo.RXTopAddress = RAMDEST_UART0_RX_END;
	UART_RXinfo.RXMaxFrameSize = FIFOED_AVALON_UART_0_RX_FIFO_SIZE;
	UART_RXinfo.RXTimeStampSize = TIMESTAMP_SIZE;
	UART_RXinfo.RXFrameLengthSize = FRAME_LENGTH_SIZE;
	UART_RXinfo.RXNextFrameAddress = RAMDEST_UART0_RX_START;
	channelRXBaudRate = FIFOED_AVALON_UART_0_BAUD;
	channelRXGapDetectionChars = FIFOED_AVALON_UART_0_GAP_VALUE;
	...
	alt_msgdma_construct_standard_mm_to_mm_descriptor(
			...
			RAMDEST_UART_RXINFO_UART0,      /* write address */
